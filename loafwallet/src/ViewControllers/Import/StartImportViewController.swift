...
class StartImportViewController : UIViewController {
    ...
    private func handleData(data: [[String: Any]], key: BRKey) {
        ...
        balanceActivity.dismiss(animated: true, completion: {
            ...
            let balanceText = self.store.state.isSumSwapped ? balanceAmount.localCurrency : balanceAmount.bits
            let feeText = self.store.state.isSumSwapped ? feeAmount.localCurrency : feeAmount.bits
