...
class Sender {
    ...
    private func verifyPin(tx: BRTxRef, withFunction: (@escaping(String) -> Bool) -> Void, completion:@escaping (SendResult) -> Void) {
        withFunction({ pin in
            ...
            if result == .timedOut {
                ...
                let alert = UIAlertController(title: S.Sumcoin WalletAlert.corruptionError, message: S.Sumcoin WalletAlert.corruptionMessage, preferredStyle: .alert)
    ...
    private func postProtocolPaymentIfNeeded() {
        ...
        request.setValue("application/sumcoin-payment", forHTTPHeaderField: "Content-Type")
        request.addValue("application/sumcoin-paymentack", forHTTPHeaderField: "Accept")
        ...
        URLSession.shared.dataTask(with: request as URLRequest) { data, response, error in
            ...
            if response.mimeType == "application/sumcoin-paymentack" && data.count <= 50000 {
