...
struct HideStartFlow : Action {
    let reduce: Reducer = { state in
        return ReduxState(isStartFlowVisible: false,
                     ...
                     isSumSwapped: state.isSumSwapped,
...
enum CurrencyChange {
    struct toggle: Action {
        let reduce: Reducer = {
            UserDefaults.isSumSwapped = !$0.isSumSwapped
            return $0.clone(isSumSwapped: !$0.isSumSwapped)
        }
    }
}
...
extension ReduxState {
    ...
    func clone(isSumSwapped: Bool) -> ReduxState {
        return ReduxState(isStartFlowVisible: isStartFlowVisible,
                     isLoginRequired: isLoginRequired,
                     rootModal: rootModal,
                     walletState: walletState,
